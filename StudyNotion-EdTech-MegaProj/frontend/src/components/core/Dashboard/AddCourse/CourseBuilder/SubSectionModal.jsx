...
export default function SubSectionModal({
  ...
  const handleEditSubsection = async () => {
    ...
    setLoading(true)
    try {
      const res = await apiConnector("PUT", course_sections.EDIT_SUB_SECTION, formData, {
      ...
      if (res?.data?.success) {
        ...
        const updatedCourseContent = (course.courseContent || []).map((section) => {
          if (String(section._id) !== String(modalData.sectionId)) return section
          // map subSection array and replace the edited one
          const updatedSubArr = (section.subSection || []).map((ss) =>
            String(ss._id) === String(updatedSubSection._id) ? updatedSubSection : ss
          )
  ...
  const onSubmit = async (data) => {
    ...
    setLoading(true)
    try {
      const res = await apiConnector("POST", course_sections.CREATE_SUB_SECTION, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      })
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 px-8 py-10">
          ...
          <div className="flex flex-col space-y-2">
            <label className="text-sm text-richblack-5" htmlFor="lectureTime">
              Lecture Duration (e.g. 00:15:30) {!view && <sup className="text-pink-200">*</sup>}
            </label>
            <input
              disabled={view || loading}
              id="lectureTime"
            ...
            />
            {errors.lectureTime && (
              <span className="ml-2 text-xs tracking-wide text-pink-200">Lecture duration is required</span>
            )}
          </div>
